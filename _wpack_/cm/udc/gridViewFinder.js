...
scwin.searchManager = {
  onceProcCnt: 1000,
  // 검색 프로세스의 1회 동작당 처리할 행의 개수를 설정한다.
  procId: -1,
  // 프로세스 timer id
  procInterval: 10,
  // 프로세스 timer interval
  ...
  procFunc: function () {
    ...
    let dispText;
    let data;
    if (r == 0) scwin.emit_onSearchProgress(grd, scwin.keyword, [], r, 0);
    for (let i = r, cnt = 0; i < r + l; i++, cnt++) {
      ...
      for (let j = 0; j < cc; j++) {
        if (!stopProc) {
          ...
          if (!$c.util.isEmpty($p, data) && re.test(data)) {
            scwin.arr.push([i, j]);
          }
        }
    ...
    if (curText != dispText) txb_result.setValue(curText = dispText);
  }
};
...
scwin.onpageload = function () {
  ...
  ipt_search.bind('onkeyup', function (e) {
    ...
    if ($c.util.isEmpty($p, keyword)) {
      scwin.clearSearch();
    } else {
      if (e.shiftKey && e.keyCode == 13 || e.keyCode == 38) {
        // Arrow Up
        scwin.btn_up_onclick();
      } else if (e.keyCode == 40) {
        //shift + enter, 	Arrow Down        
...
scwin.setHotKeyInfo = function (v) {
  ...
  scwin.hotKeyInfo.complexKey = hkeys.sort((a, b) => {
    return complexKeyOrder.indexOf(a) - complexKeyOrder.indexOf(b);
  });
};
...
scwin.checkKey = function (e = {}) {
  ...
  if (optLastKey === lastKey) {
    ...
    if (fireHotKey && optComplexKey.length == 0) {
      if (f_number.indexOf(lastKey) > -1)
        //single key는 F1~F12 만 허용
        fireHotKey = true;else fireHotKey = false;
    }
  }
  return fireHotKey;
};
...
scwin._getRootComponent = function (elem) {
  ...
  function findComp(p) {
    ...
    if (p.id) {
      ...
      if (p.parentElement) return findComp(p.parentElement);else return;
    }
...
scwin.searchBoxShow = function (e) {
  ...
  const status = $c.util.isEmpty($p, comObj);
...
scwin.moveToBox = function (comObj, noShow) {
  ...
  if ($c.util.isEmpty($p, comObj)) {
    ...
    grid = $c.win.getParent($p).$p.getComponentById(comObj);
  }
...
scwin.collectGridView = function (parent) {
  const frame = $c.win.getFrame($p, parent.$p);
  scwin.gridInfo = WebSquare.util.getChildren(frame, {
    recursive: true,
...
scwin.btn_down_onclick = function (e) {
  if (btn_down.getDisabled()) return;
...
scwin.dpFormatter = function (data) {
  ...
  for (let col of scwin.displayOpt) {
    ...
    if (col.g == scwin.gIdx && col.c == scwin.cIdx && !$c.util.isEmpty($p, col.displayFormatter)) {
      // 함수 찾고
    ...
    for (let i = 0; i < cnt; i++) {
      ...
      for (let j = 0; j < colCnt; j++) {
        ...
        if (type == 'select' || type == 'autoComplete') {
          ...
          const dltSet = $c.win.getParent($p).$p.getComponentById(colDltInfo.id).getAllJSON();
...
scwin.invokeFormatter = function (data, formattedData, rowIdx, colIdx) {
  for (let col of scwin.displayOpt) {
    ...
    if (col.g == scwin.gIdx && col.c == colIdx && !$c.util.isEmpty($p, col.displayFormat)) {
      data = formattedData;
    }
    ...
    // displayFormatter가 있는 경우 Data 적용
    if (col.g == scwin.gIdx && col.c == colIdx && !$c.util.isEmpty($p, col.displayFormatter)) {
      // 함수 찾고
      const dpFunc = scwin.findDisplayFormatter(col.displayFormatter);
      // 데이터 적용
      data = dpFunc(data);
...
}}}]}]},{T:1,N:'body',A:{'ev:onpageload':'scwin.onpageload',class:'w2grid_finder'},E:[{T:1,N:'xf:group',A:{id:'grp_wrap',class:'w2grid_finder gvw_finder'},E:[{T:1,N:'xf:group',A:{style:'',id:'',class:'w2grid_finder_form'},E:[{T:1,N:'xf:input',A:{style:'width: 200px;height: 30px;',id:'ipt_search'}},{T:1,N:'w2:textbox',A:{tagname:'p',style:'',id:'txb_result',label:'[0/0]',class:'message'}},{T:1,N:'w2:textbox',A:{tagname:'button',style:'',id:'btn_up',label:'<',class:'btn_movePrevious','ev:onclick':'scwin.btn_up_onclick'}},{T:1,N:'w2:textbox',A:{tagname:'button',style:'',id:'btn_down',label:'>',class:'btn_moveNext','ev:onclick':'scwin.btn_down_onclick'}},{T:1,N:'w2:textbox',A:{tagname:'button',style:'',id:'btn_close',label:'X',class:'btn_finder_close','ev:onclick':'scwin.btn_close_onclick'}}]}]}]}]}]})