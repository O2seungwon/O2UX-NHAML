/*amd /cm/gcc/str.xml 29067 afa3ba612747b97d286cca2275ddd0559de1db2b5f5d32bd03e9ac6091be5251 */
define({declaration:{A:{version:'1.0',encoding:'UTF-8'}},E:[{T:1,N:'html',A:{xmlns:'http://www.w3.org/1999/xhtml','xmlns:ev':'http://www.w3.org/2001/xml-events','xmlns:w2':'http://www.inswave.com/websquare','xmlns:xf':'http://www.w3.org/2002/xforms'},E:[{T:1,N:'head',A:{},E:[{T:1,N:'w2:type',E:[{T:3,text:'COMMON'}]},{T:1,N:'w2:buildDate'},{T:1,N:'w2:MSA'},{T:1,N:'xf:model',E:[{T:1,N:'w2:dataCollection',A:{baseNode:'map'}},{T:1,N:'w2:workflowCollection'}]},{T:1,N:'w2:layoutInfo'},{T:1,N:'w2:publicInfo',A:{method:'scwin.serialize,scwin.lpad,scwin.rpad,scwin.formatSSN,scwin.formatTime,scwin.getLocale,scwin.existKorean,scwin.isKorean,scwin.isFinalConsonant,scwin.attachPostposition,scwin.isBizID,scwin.isSSN,scwin.isEmail,scwin.replaceAll,scwin.trim,scwin.getByteLength,scwin.maskEndString,scwin.maskEmail,scwin.maskPassPort,scwin.maskCardNo,scwin.maskCusNo,scwin.formatMappedAcno,scwin.formatAcno,scwin.formatAcnoMask,scwin.formatAcnoForSbjc,scwin.formatAcnoLpad,scwin.formatRlnoMasked,scwin.formatTelNo,scwin.formatTelNoMask'}},{T:1,N:'script',A:{lazy:'false',type:'text/javascript'},E:[{T:4,cdata:function(scopeObj){with(scopeObj){/**
 * @component
...
scwin.attachPostposition = function ($p, str) {
  if ($c.win.getLanguage($p) === "ko") {
    if (scwin.isFinalConsonant($p, str)) {
      str = str + "은";
    } else {
      str = str + "는";
    }
 ...
 */
scwin.maskEndString = function ($p, str, maskLen, char) {
  // $p, str, mask, char
  if (typeof $p == 'string') {
    char = maskLen;
    maskLen = str;
  ...
  if ($c.util.isEmpty($p, char)) char = "*";
  if ($c.util.isEmpty($p, scwin.trim($p, str))) return str;
  if ($c.util.isEmpty($p, maskLen)) maskLen = Math.round(str.length / 2); //없으면 문자열 절반 마스킹
  if (str.length <= maskLen) return str; //마스킹처리 길이보다 작으면 return

  return scwin.rpad($p, str.substring(0, str.length - maskLen), str.length, char);
};
 ...
 */
scwin.maskEmail = function ($p, str) {
  if (!scwin.isEmail($p, str)) return str;
  const splitedEmail = str.split("@");
  const email = splitedEmail[0];
  const domain = splitedEmail[1];
  if (email.length < 4) return str;
  const maskEmail = scwin.maskEndString($p, email, email.length - 3);
 ...
 */
scwin.maskPassPort = function ($p, str) {
  if ($c.util.isEmpty($p, str)) return "";
  if (str.length < 5) return str;
  const rtnStr = scwin.maskEndString($p, str, 4, "*");
  return rtnStr;
};
 ...
 */
scwin.maskCardNo = function ($p, str) {
  const cardNo = scwin.trim($p, str)?.replaceAll("-", "");
  let rtnStr = "";
  if ($c.util.isEmpty($p, cardNo)) return str;
  if (cardNo.length == 15) {
 ...
 */
scwin.maskCusNo = function ($p, str) {
  const cusNo = scwin.trim($p, str)?.replaceAll("-", "");
  if (cusNo.length < 2) return str;
  const len = cusNo.length > 5 ? 5 : Math.round(cusNo.length / 2);
  const rtnStr = scwin.maskEndString($p, cusNo, len, "*");
  return rtnStr;
};
 ...
 * @example 
 */
scwin.formatMappedAcno = function ($p, acnoStr) {
  let retStr = "";
 ...
 */
scwin.formatAcno = function ($p, str) {
  let accNo = scwin.trim($p, str);
  if ($c.util.isEmpty($p, accNo) || !accNo.replaceAll("0", "")) return "";
 ...
 */
scwin.formatAcnoMask = function ($p, str) {
  const accNo = scwin.trim($p, str);
  if ($c.util.isEmpty($p, accNo) || !accNo.replaceAll("0", "")) return "";
  const maskAccNo = scwin.maskEndString($p, accNo, 6);
  const rtnAcc = scwin.formatAcno($p, maskAccNo);
  return rtnAcc;
};
 ...
 * @example 
 */
scwin.formatAcnoForSbjc = function ($p, pAcno, pAcSbjc, pAudMbAcno, pAcBrc, pBnknm, pCkno) {
  const delimeter = "-";
  if (pAcSbjc == '136') {
    return pAcBrc + '-' + pAcSbjc + '-' + pCkno;
  }
  if (pAcSbjc == '210') {
    if (scwin.trim($p, pAudMbAcno) != '' && pAudMbAcno.length >= 8) {
      return pAcBrc + '-' + pAcSbjc + '-' + pAudMbAcno.substring(0, 8);
    }
    return scwin.formatAcno($p, pAcno);
  }
 ...
 */
scwin.formatAcnoLpad = function ($p, pAcno1, pAcno2, pAcno3) {
  const lAcno1 = $c.num.parseInt($p, scwin.trim($p, pAcno1));
 ...
 */
scwin.formatRlnoMasked = function ($p, str, pIsMask = 'N') {
  let retVal = "";
  let lRlno = scwin.trim($p, str.replaceAll('-', ''));
  ...
  if (lRlno.length == 13) {
    // 주민번호, 법인번호 6-7
    if (pIsMask !== 'N') lRlno = scwin.maskEndString($p, lRlno, 7);
    retVal = lRlno.substring(0, 6) + "-" + lRlno.substring(6, 13);
  } else if (lRlno.length == 10) {
    // 사업자번호 3-2-5 
    if (pIsMask !== 'N') lRlno = scwin.maskEndString($p, lRlno, 5);
 ...
 */
scwin.formatTelNo = function ($p, str) {
  if (typeof $p == 'string') {
    str = $p;
  }
  str = $c.str.trim($p, str).replaceAll('-', '');
  let rtnStr = "";
 ...
 */
scwin.formatTelNoMask = function ($p, str) {
  if (typeof $p == 'string') {
    str = $p;
  }
  str = scwin.trim($p, str).replaceAll('-', '');
  if (str.length < 8 || str.length > 13) return str;
  if (str.length == 8) {
    rtnStr = str.replace(/(\d{4})(\d{4})/, "$1-$2");
  } else if (str.length == 10) {
