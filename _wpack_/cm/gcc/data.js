...
scwin.COMMON_CODE_INFO = {
  ...
  //[코드] 코드명 으로된 컬럼 추가
  VALUE: "C_VAL",
  QUERY_ID: "SS01010_S003",
  FILED_ARR: ["BSN_DSC_C_ID", "BSN_DSC", "C_ID", "C_VAL", "CNM", "VAL_CNM", "INTGC_YN", "UYN", "SORT_SQ", "HOFC_ACS_YN", "ETCRF_CNTN1", "ETCRF_CNTN2", "ETCRF_CNTN3", "ETCRF_CNTN4", "ETCRF_CNTN5", "ETCRF_CNTN6", "ETCRF_CNTN7", "ETCRF_CNTN8", "ETCRF_CNTN9"]
};
...
scwin.setDownloadGridViewOption = function ($p, grdObj, options) {
  ...
  if ($c.util.isEmpty($p, fileName)) {
    let dataCollectionId = scwin.getDataCollection($p, grdObj).id;
  ...
  function getDataHandlerData(grdObj, opts) {
    try {
      ...
      $c.win.alert($p, "엑셀 파일이 정상적으로 생성되지 않았습니다.");
...
scwin.initChangeCheckedDc = function ($p) {
  ...
  if (!$c.util.isEmpty($p, scwin.getParameter($p, "menuInfo"))) {
...
scwin.setChangeCheckedDc = function ($p, dcObjArr) {
  const mainFrameScwin = scwin.getChangeCheckedMainFrame($p);
...
scwin.__getChangeCheckedMainFrame = function (scopeApi) {
  ...
  if (typeof scwinObj._changeCheckDcList !== "undefined") {
    ...
    scopeApi = $p.parent().$p;
    return scwin.getChangeCheckedMainFrame($p, scopeApi);
  } else {
    return null;
  }
};
 ...
 * @param {String} codeOptions.dltName 공통코드 dataList 를 지정한 명칭으로 생성
 * @param {Boolean} codeOptions.useYn 사용여부가 Y인 세부코드만 조회
 * @param {Array} codeOptions.filter 데이터리스트를 해당 조건으로 filter 처리.
 * @param {String} codeOptions.filter.col filter처리할 컬럼명
 * @param {String} codeOptions.filter.value filter처리할 컬럼명의 값
 * @param {requestCallback} callbackFunc 콜백 함수
 * @hidden N
 * @exception 
 * @example 
 * const codeOptions = [{ bsnCd: "SS", code : "0001",  compID : "sbx_Duty" },
                        { bsnCd: "SS", code : "0002",  compID : "sbx_Postion", dltName : "SS0002_test" },
                        { bsnCd: "SS", code : "00005", compID : "sbx_CommCodePart1, sbx_CommCodePart2"},
                        { bsnCd: "SS", code : "00024", compID : "grd_CommCodeSample:JOB_CD"} 
                        { bsnCd: "SS", code : "9992",  compID : "sbx_group_ex01", useYn : true, filter : [{col : "ETCRF_CNTN1", value : "1"}, {col : "ETCRF_CNTN2", value : "N"}]},
                        ];
...
scwin.setCommonCode = function ($p, codeOptions, callbackFunc) {
  ...
  for (i = 0; i < codeOptionsLen; i++) {
    ...
    try {
      dltId = scwin.DATA_PREFIX + codeObj.bsnCd + codeObj.code;
      if (!$c.util.isEmpty($p, codeObj.dltName)) {
        dltId = scwin.DATA_PREFIX + codeObj.dltName;
      }
      ...
      if (typeof scwin.commonCodeList[dltId] === "undefined") {
        ...
        //     paramCode += ",";
        // }
        // paramCode += codeObj.bsnCd + codeObj.code;
        // paramCode[codeObj.bsnCd + codeObj.code] = {"bsnCd": codeObj.bsnCd, "code" : codeObj.code};
        paramCode.push(codeObj.bsnCd + codeObj.code);
          ...
          if (tmpIdArr.length === 1) {
            ...
            if (!$c.util.isEmpty($p, comp)) {
              if (comp.getPluginName() === "selectbox") {
                //bbok
                comp.setNodeSet("data:" + dltId, scwin.COMMON_CODE_INFO.FULLABEL, scwin.COMMON_CODE_INFO.VALUE);
              } else {
  ...
  const searchCodeGrpOption = {
    id: "_sbm_searchCode",
    action: "/db-query-execute/select.ml",
    target: "data:json," + $c.str.serialize($p, dltIdArr),
    isProcessMsg: false
  };
  searchCodeGrpOption.submitDoneHandler = function (e) {
    const dltAllObj = $c.util.getComponent($p, scwin.DATA_PREFIX);
    dltAllObj.setJSON(e.responseJSON);
    for (let i = 0; i < dltAllObj.getRowCount(); i++) {
      // dltAllObj.setCellData(i, "BSN_DSC_C_ID", dltAllObj.getCellData(i, "BSN_DSC") + dltAllObj.getCellData(i, "C_ID"));
      dltAllObj.setCellData(i, "VAL_CNM", "[" + dltAllObj.getCellData(i, "C_VAL") + "] " + dltAllObj.getCellData(i, "CNM"));
    }
    for (let i = 0; i < codeOptions.length; i++) {
      const bsnCid = codeOptions[i].bsnCd + codeOptions[i].code;
      const dataList = codeOptions[i]?.dltName ? codeOptions[i].dltName : bsnCid;
      const codeJSON = dltAllObj.getMatchedJSON("BSN_DSC_C_ID", bsnCid, true);
      const dltObj = $c.util.getComponent($p, scwin.DATA_PREFIX + dataList);
      dltObj.setJSON(codeJSON);

      //useYn이 true 이면 사용여부가 Y인 자료만 나오도록 filter처리
      if (codeOptions[i].useYn === true) {
        dltObj.setColumnFilter({
          type: 'row',
          colIndex: 'UYN',
          key: "1",
          condition: 'and',
      ...
      //filter가 존재하는 경우... 
      if (!$c.util.isEmpty($p, codeOptions[i].filter)) {
        if (Array.isArray(codeOptions[i].filter)) {
          for (let f = 0; f < codeOptions[i].filter.length; f++) {
            const filterInfo = codeOptions[i].filter[f];
            dltObj.setColumnFilter({
              type: 'func',
              condition: 'and',
              colIndex: filterInfo.col,
              key: _dataFilterFunction,
              param: filterInfo.value
            });
          }
          //김충완 부장 추가 -> dataList를 재정의 하는 방식으로 dataList.clearFilter()가 되지 않음 주의(비권장)
        } else if (typeof codeOptions[i].filter === 'function') {
          const d = dltObj.getAllJSON().filter(codeOptions[i].filter);
          dltObj.setJSON(d);
        }
      }
    }
  ...
  if (paramCode !== "") {
    ...
    const reqData = {
      "dma_commonCode": {
        "BSN_DSC_C_ID": paramCode,
        "query_id": scwin.COMMON_CODE_INFO.QUERY_ID
      }
    };
  ...
  function _dataFilterFunction(cellData, tmpParam, rowIdx) {
    // cellData는 웹스퀘어 엔진에서 넘겨주는 셀의 데이터로, colIndex에 설정한 컬럼의 데이터
    // tmpParam은 setColumnFilter() API를 호출 할 때 넘겨주는 파라미터 값
    // rowIdx는 웹스퀘어 엔진에서 넘겨주는 행의 인덱스
    if (cellData == tmpParam) {
      return true;
    } else {
...
scwin.downloadGridViewExcel = function ($p, grdObj, options, infoArr) {
  ...
  scwin.setDownloadGridViewOption($p, grdObj, opts);
...
scwin.downloadGridViewCSV = function ($p, grdObj, options) {
  ...
  scwin.setDownloadGridViewOption($p, grdObj, options);
  const opts = {
...
scwin.validateGroup = function ($p, grpObj, valInfoArr, tacObj, tabId) {
  ...
  try {
    for (let objIdx in objArr) {
      ...
      const dataObjInfo = scwin.getDataCollection($p, obj);
      let dataCollection = null;
      let columnId = null;
      let value = null;
    ...
    function _setResult(dataCollection, objId, valInfo, value) {
      const msgInfo = scwin.getValResultMsg($p, valInfo, value);
      if ($c.util.isEmpty($p, msgInfo.message) === false) {
        ...
        if ($c.util.isEmpty($p, valInfo.label) === false) {
          ...
          errors[errIdx].columnName = scwin.getColumnName($p, comObj);
...
scwin.validateGridView = function ($p, gridViewObj, valInfoArr, tacObj, tabId) {
  ...
  try {
    ...
    function _setResult(rowIndex, dataList, gridViewObjId, valInfo, value) {
      const msgInfo = scwin.getValResultMsg($p, valInfo, value, dataList, rowIndex);
      if ($c.util.isEmpty($p, msgInfo.message) === false) {
        const errIdx = errors.length;
        errors[errIdx] = {};
        errors[errIdx].columnId = valInfo.id;
...
scwin.undoGridView = async function ($p, grdId) {
  try {
    ...
    if (typeof grdObj === "object" && grdObj !== "") {
      ...
      if (scwin.isModified($p, dltObj)) {
        if (await $c.win.confirm($p, "변경된 정보가 있습니다. 저장하지 않고 취소하시겠습니까?")) {
          scwin.undoAll($p, dltObj);
        }
        ;
      }
      ;
...
scwin.getMatchedJSON = function ($p, dataListObj, condArr) {
  ...
  for (let idx = 0; idx < allData.length; idx++) {
    ...
    if (result === true) {
      allData[idx].rowIndex = idx; //2025.09.26 rowIndex 정보도 같이 추가
...
scwin.createHolidayDataList = function ($p) {
  ...
  scwin.createDataList($p, "dlt_holiday", ["REST_DATE", "REST_NAME", "REST_CODE", "NOTE", "REST_SEQ"], ["text", "text", "text", "text", "text"], dcOption);
 ...
 * @method
 * @name validateDtPrd
 * @description 기간지정날짜에 대한 유효성체크
 * @param {String} pStDt 시작일
 * @param {String} pEdDt 종료일
 * @param {String} pPrdNm 메세지처리를 위한 기간명
 * @param {String} pMaxNum 최대기간 일수
 * @returns {Boolean} 유효성 여부
 * @hidden N
 * @exception 
 * @example
 */
scwin.validateDtPrd = function ($p, pStDt, pEdDt, pPrdNm, pMaxNum = '') {
  let message = "";
  if ($c.util.isEmpty($p, pStDt)) {
    message = `${pPrdNm} 시작일을 입력하십시오.`;
  } else if (pStDt?.length == 8) {
    if (pStDt == '00000000') {
      message = `${pPrdNm} 시작일을 입력하십시오.`;
    } else if (pEdDt == '00000000' || $c.util.isEmpty($p, pEdDt)) {
      message = `${pPrdNm} 종료일을 입력하십시오.`;
    } else if (IsValidDate(pStDt) != 1 || IsValidDate(pEdDt) != 1) {
      message = `${pPrdNm} 일자를 바르게 입력하십시오.`;
    } else if (pStDt > pEdDt) {
      message = `${pPrdNm} 일자를 확인하십시오.\n\n시작일이 종료일보다 이후 날짜입니다.`;
    } else if (pMaxNum && SpanDay(pStDt, pEdDt) + 1 > pMaxNum) {
      message = `대량 데이타 발생으로 인하여 조회가 불가 합니다.\n\n${pPrdNm}을 ${pMaxNum}일 이내로 설정하고 조회하세요.`;
    }
  } else if (pStDt?.length == 6) {
    if (pStDt == '000000') {
      message = `${pPrdNm} 시작년월을 입력하십시오.`;
    } else if ($c.util.isEmpty($p, pEdDt) || pEdDt == '000000') {
      message = `${pPrdNm} 종료년월을 입력하십시오.`;
    } else if (IsValidDate(pStDt + '01') != 1 || IsValidDate(pEdDt + '01') != 1) {
      message = `${pPrdNm} 년월을 바르게 입력하십시오.`;
    } else if (pStDt > pEdDt) {
      message = `${pPrdNm} 년월을 확인하십시오.\n\n시작년월이 종료년월보다 이후 날짜입니다.`;
    }
  } else if (pStDt?.length == 4) {
    if (pStDt == '0000') {
      message = `${pPrdNm} 시작년도를 입력하십시오.`;
    } else if ($c.util.isEmpty($p, pEdDt) || pEdDt == '0000') {
      message = `${pPrdNm} 종료년도를 입력하십시오.`;
    } else if (pStDt > pEdDt) {
      message = `${pPrdNm} 년도를 확인하십시오.\n\n시작년도가 종료년도보다 이후 입니다.`;
    }
  }
  if (!$c.util.isEmpty($p, message)) {
 ...
 * @method
 * @name initComponents
 * @description group 영역의 컴포넌트 초기화
 * @param {Object} grpObj 초기화할 영역의 group 컴포넌트
 * @param {String} excludeIdStr 초기화 기능에서 제외되는 컴포넌트ID. 복수 개를 정의할 경우 공백(" ")을 구분자로 사용.
 * @returns 
 * @hidden N
 * @exception 
 * @example
 * $c.data.initComponents(grp_search);
 * $c.data.initComponents(grp_search, "ibx_title cal_st_dt");
 */
scwin.initComponents = function ($p, grpObj, excludeIdStr) {
  //초기화가 가능한 컴포넌트: Calendar, Checkbox, GridView, Input, InputCalendar, MultiSelect, Radio, SelectBox, TextArea
  try {
    let excludeIdStr = "";
    if (!$c.util.isEmpty($p, excludeIdStr)) {
      const excludeId = excludeIdStr.split(" ");
      //컴포넌트.id 으로 들어가야 정상동작하기 때문에 한번 풀어줌.
      for (let i = 0; i < excludeId.length; i++) {
        const comp = $c.util.getComponent($p, excludeId[i]);
        if (!$c.util.isEmpty($p, comp)) {
          excludeIdStr = (i === 0 ? "" : " ") + comp.id;
        }
      }
    }

    //만약에 grpObj 내에 udc가 존재하는 경우
    const childObj = $c.util.getChildren($p, grpObj, {
      excludePlugin: "output textbox group body",
      //제외할 컴포넌트 plugin 이름.
      recursive: true
    });
    let udcId = [];
    for (let c = 0; c < childObj.length; c++) {
      if (childObj[c].scope_obj._isUDC2) {
        const udcObjId = $c.util.getComponent($p, childObj[c].parentFrame).udc2Parent.org_id;
        if (udcId.indexOf(udcObjId) < 0) {
          udcId.push(udcObjId);
        }
        $c.util.getComponent($p, childObj[c].parentFrame).id;
      }
    }
    if (udcId.length > 0) {
      for (let u = 0; u < udcId.length; u++) {
        $c.util.getComponent($p, udcId[u]).setInit(); //udc 초기화
      }
    }
    const options = {
      excludeId: excludeIdStr //초기화 기능에서 제외되는 컴포넌트 ID
      ,

      fireEvent: false
    }; //초기화로 인한 값 변동에 따라 이벤트 발생 여부. (기본값: false. 이벤트 발생을 방지.)

    grpObj.init(options);
  } catch (e) {
    console.log("[error] scwin.initComponents : " + e.message);
  }
};
 ...
 * @method
 * @name setUserInfo
 * @description userInfo 정보를 조회해서 전역변수에 담음.
 * @param {Object} dataInfo 호출된 userInfo 정보
 * @returns 
 * @hidden N
 * @exception 
 * @example
 * $c.data.setUserInfo(dataInfo);
 */
scwin.setUserInfo = function ($p, dataInfo) {
  if (!$c.util.isEmpty($p, dataInfo)) {
    $c.data.userInfo = dataInfo;
  }
};
 ...
 * @method
 * @name getUserInfo
 * @description 전역변수에 저장된 userInfo 정보를 조회.
 * @param {string} data userInfo 에 저장된 key
 * @returns {String|Array} 해당 key값의 value
 * @hidden N
 * @exception 
 * @example
 * $c.data.getUserInfo(brnm);   //return "IT부문";
 */
scwin.getUserInfo = function ($p, data) {
  if ($c.util.isEmpty($p, data)) {
    return $c.data.userInfo;
  } else {
    const rtnValue = $c.data.userInfo[data];
    if ($c.util.isEmpty($p, rtnValue)) {
      return "";
    } else {
      if (data == "grp_cds" || data == "grpnm") {
        return rtnValue.split(",");
      }
    }
    return rtnValue;
  }
 ...
 * @method
 * @name hasAnyGroupCds
 * @description 그룹코드가 존재하는지 여부 체크
 * @param {string} groupCds 비교할 그룹코드.복수 개를 정의할 경우 공백(",")을 구분자로 사용.
 * @returns {Boolean} 일치여부
 * @hidden N
 * @exception 
 * @example
 * $c.data.hasAnyGroupCds("00");   //return true;
 * $c.data.hasAnyGroupCds("00,09");   //return false;
 */
scwin.hasAnyGroupCds = function ($p, groupCds) {
  const searchCd = groupCds.split(",");
  const chkArray = scwin.getUserInfo($p, "grp_cds");
  for (let ck = 0; ck < chkArray.length; ck++) {
    for (let sh = 0; sh < searchCd.length; sh++) {
      if (searchCd[sh] == chkArray[ck]) {
        return true;
      }
    }
  }
 ...
 * @method
 * @name setServerInfo
 * @description server 정보를 조회해서 전역변수에 담음.
 * @param {Object} server 정보
 * @returns 
 * @hidden N
 * @exception 
 * @example
 * $c.data.setServerInfo(dataInfo);
 */
scwin.setServerInfo = function ($p, dataObj) {
  if (!$c.util.isEmpty($p, dataObj)) {
    $c.data.serverInfo = dataObj;
  }
};
 ...
 * @method
 * @name getServerType
 * @description server type 정보를 조회
 * @param 
 * @returns {String} server 정보
 * @hidden N
 * @exception 
 * @example
 * $c.data.getServerType(dataInfo);
 */
scwin.getServerType = function ($p) {
  return $c.data.serverInfo?.active || "local";
};

/**
 * @method
 * @name fnInsertInfPrttInq
 * @description 정보보호 조회내역 등록
 * @param {String} pSchRlno 조회조건으로 사용된 실명번호
 * @param {String} pPgId 조회화면의 프로그램ID
 * @param {String} pPgNm 조회화면의 프로그램명
 * @returns {Boolean} ture - 정상처리,  false - 비정상종료
 * @hidden N
 * @exception 
 * @example
 * $c.data.fnInsertInfPrttInq("8101011111111", "SAMPLE01", "샘플화면");
 */
scwin.fnInsertInfPrttInq = function ($p, pSchRlno, pPgId, pPgNm) {
  console.log('======================= pSchRlno = ' || pSchRlno);

  //임직원여부 확인
  return new Promise(function (resolve, reject) {
    if ($c.util.isEmpty($p, pSchRlno)) {
      resolve(false);
    }
    let param = {
      query_id: "COM02_INFPRTT_S001",
      pSchRlno: pSchRlno
    };
    const sbm_infprTTS001 = {
      id: "sbm_infprTTS001",
      actionType: "select",
      isProcessMsg: false,
      submitDoneHandler: function (res) {
        if (res.responseJSON.length > 0) {
          $c.win.confirm($p, "자금세탁방지 업무 외의 사유로 임직원의 정보를 조회하는 경우, 소명서 작성 대상이 됩니다.\n\n계속 조회하시겠습니까?").then(rtn => {
            if (rtn) {
              let param = {
                query_id: "COM02_INFPRTT_I001",
                pSchRlno: pSchRlno,
                pPgId: pPgId,
                pPgNm: pPgNm,
                "INQ_OBJ_ENO": res.responseJSON[0].INQ_OBJ_ENO,
                "SS_USER_IP": '16.24.144.115:38790' //기본값 설정
              };
              const sbm_insert_I001 = {
                id: "sbm_insert_I001",
                actionType: "insert/",
                isProcessMsg: false,
                submitDoneHandler: function (res) {
                ...
                submitErrorHandler: function (e) {
                  $c.win.alert($p, "임직원 조회이력 등록 처리중 오류");
                  resolve(false);
                }
              };
              $c.sbm.executeSubmit($p, sbm_insert_I001, param);
            } else {
              resolve(false);
            }
          });
    ...
    $c.sbm.executeSubmit($p, sbm_infprTTS001, param);
  });
};
