...
scwin.__callbackSubmitFunction = function (resObj, sbmObj) {
  ...
  if (resObj.responseStatusCode < 100 || resObj.responseStatusCode > 599) {
    ...
    scwin.resultMsg($p, msgObj);
    return false;
  }

  //bbok
  if (sbmObj.userData1) {
    const dataObj = $c.util.getComponent($p, sbmObj.userData1);
    if ($c.util.isEmpty($p, dataObj)) {
      console.error("target(" + sbmObj.userData1 + ")에 해당하는 dataCollection 이 존재하지 않습니다.");
    } else {
      if (resObj?.responseJSON.length > 0 || $c.util.isPlainObject($p, resObj?.responseJSON)) {
        dataObj.setJSON(resObj.responseJSON);
      } else {
        dataObj.setJSON([]);
      }
    }
  }
  if (!$c.util.isEmpty($p, sbmObj.searchResult)) {
    //조회조건 count
    const searchInfo = $c.util.getComponent($p, sbmObj.searchResult);
    searchInfo.setValue(resObj?.responseJSON.length);
  }
  ...
  if (rsJSON && rsJSON.rsMsg) {
    scwin.resultMsg($p, rsJSON.rsMsg);
  }
};
...
scwin.__submitErrorHandler = function (resObj) {
  ...
  // let messageStr = JSON.parse(resObj.responseBody)?.message || "서버 오류입니다. 자세한 내용은 관리자에게 문의하시기 바랍니다.";
  ...
  if (typeof $c.util.getComponent($p, resObj.id).submitErrorHandler === "function") {
    $c.util.getComponent($p, resObj.id).submitErrorHandler();
  }
};
  ...
  return new Promise(function (resolve, reject) {
    if (typeof sbmObj === "object") {
      ...
      if ($c.util.isEmpty($p, _sbmObj.action)) {
        const alertMsg = "action 은(는) 필수입력값입니다.";
 ...
 * @method
 * @name executeSubmit
 * @description Submission 생성해서 실행합니다. (NH)
 ...
 */
scwin.executeSubmit = function ($p, options, requestData, obj) {
  //bbok NH에서 submit 호출 시
  if ($c.util.isEmpty($p, options.actionType) && $c.util.isEmpty($p, options.action)) {
    $c.win.alert($p, "actionType 또는 action 정보가 필요합니다.");
    return false;
  }
  let userData1 = "";
  if ($c.util.isEmpty($p, options.action) && !$c.util.isEmpty($p, options.actionType)) {
    options.action = `/db-query-execute/${options.actionType}.ml`;
    if (options.actionType == "select") {
      options.ref = `data:json,${options.ref}`;
    } else {
      options.ref = `data:json,{"action":"${options.actionType}","id":"${options.ref}"}`;
    }
  } else {
    //options.action 이 존재하는 경우
    options.ref = `data:json,${options.ref}`;
  }
  if (!$c.util.isEmpty($p, options.target) && options.target.indexOf("data:json") < 0) {
    userData1 = options.target;
    options.target = "data:json";
  }
  if ($c.util.isEmpty($p, options.isProcessMsg)) options.isProcessMsg = true; //로딩바 보여주기 처리

  let submissionObj = $c.util.getComponent($p, options.id);
  if (submissionObj === null) {
    scwin.create($p, options);
  ...
  if (!$c.util.isEmpty($p, userData1)) {
    submissionObj.userData1 = userData1;
  }
  if (!$c.util.isEmpty($p, options.searchResult)) {
    submissionObj.searchResult = options.searchResult;
  }

  //bbok
  submissionObj.org_id = options.id;
  return scwin.execute($p, submissionObj, requestData, obj);
};
...
scwin.create = function ($p, options) {
  let {
    ...
    action = "",
    actionType = "",
    mode = scwin.DEFAULT_OPTIONS_MODE,
    mediatype = scwin.DEFAULT_OPTIONS_MEDIATYPE,
    method = "post",
    processMsg = "",
    instance = "none",
    submitHandler = "",
  ...
  submitHandler = typeof submitHandler === "function" ? submitHandler : typeof submitHandler === "string" ? submitHandler.indexOf("scwin") === 0 ? $p.id + submitHandler : submitHandler : "";
  submitDoneHandler = typeof submitDoneHandler === "function" ? submitDoneHandler : typeof submitDoneHandler === "string" ? submitDoneHandler.indexOf("scwin") === 0 ? $p.id + submitDoneHandler : submitDoneHandler : "";
  submitErrorHandler = typeof submitErrorHandler === "function" ? submitErrorHandler : typeof submitErrorHandler === "string" ? submitErrorHandler.indexOf("scwin") === 0 ? $p.id + submitErrorHandler : submitErrorHandler : "";
  let resJson = null;
  ...
  const submissionObj = {
    ...
    "method": method,
    // [default: post, get, urlencoded-post] get:파라메타를 url에 붙이는 방식 (HTML과 동일). // post:파라메타를 body 구간에 담는 방식 (HTML과 동일). urlencoded-post:urlencoded-post.
    "mediatype": mediatype,
    // application/json
    "encoding": "UTF-8",
...
scwin.getResultCode = function ($p, e) {
  let rsCode = scwin.getMessageCode($p, 'STATUS_ERROR');
  try {
  ...
  switch (resultData.statusCode) {
    case msgCode.STATUS_ERROR:
      if (resultData.errorCode == "E0001") {
        const alertMsg = `${message} 로그인 화면으로 이동하겠습니다.`;
