...
/**
 * @method 
 * @name setReadonlyComponents
 * @description 지정된 group 하위의 컴포넌트들을 일괄적으로 readOnly 처리. 
 * (setReadOnly 매소드가 없는 경우 setDisabled 처리)
 * @param {Object} grpObj readOnly처리할 컴포넌트 group 
 * @param {Boolean} bool readOnly여부
 * @param {String} excludeComponentIDArr 제외할 컴포넌트 id. 인자가 여러개일 경우 공백(" ")을 구분자로 사용.
 * @hidden N
 * @exception 
 * @example
$c.util.setReadonlyComponents(grp_ex01, false);
$c.util.setReadonlyComponents(grp_ex01, true, "chk_checkbox1 sbx_group_ex02");
 */
scwin.setReadonlyComponents = function ($p, grpObj, bool, excludeComponentIDArr) {
  try {
    var components = scwin.getChildren($p, grpObj, {
      excludePlugin: "output textbox group body",
      //제외할 컴포넌트 plugin 이름.
      recursive: true //하위컴포넌트 포함여부
    });
    for (let i = 0; i < components.length; i++) {
      var component = components[i];
      if (component.getPluginName() === "searchbox") {
        //searchbox 안의 컴포넌트가 검색되어서 오류가 남
        component = component.getParent();
      }
      var plugin = component.getPluginName().toLowerCase();
      if (!scwin.isEmpty($p, excludeComponentIDArr)) {
        var arrExcludeId = excludeComponentIDArr.split(" ");
        if ($.inArray(component.getOriginalID(), arrExcludeId) > -1) {
          continue;
        }
      }
      if (plugin == "multiupload") {
        // component.setStyle("display", "none");
      } else if (typeof component.setReadOnly == "function" && plugin != "selectbox" && plugin != "searchbox") {
        if (component.getReadOnly() != bool) {
          component.setReadOnly(bool);
        }
      } else if (typeof component.setDisabled === "function") {
        if (component.getDisabled() != bool) {
          component.setDisabled(bool);
        }
      }
    }
  } catch (e) {
    console.log("scwin.setReadonlyComponents : " + e.message);
  }
};
