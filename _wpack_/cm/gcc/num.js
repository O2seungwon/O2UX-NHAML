 ...
 * $c.num.isNumber("1a23");    // false
 * $c.num.isNumber("");        // false
 * $c.num.isNumber(12345677);  // true
 * $c.num.isNumber(-12345677); // true
 */
scwin.isNumber = function ($p, value) {
  if ($c.util.isEmpty($p, $c.str.trim($p, value))) return false; //빈값은 false
  return !isNaN(value);
};
 ...
 * @hidden N
 * @exception 
 * @example $c.num.formatByte(32132);
 */
 ...
 * @name formatUnit
 * @description 
 * @param {String} value 숫자가 아닌경우 넘어온값을 그대로 리턴
 * @param {String} unit 기본값 : 1
 * @returns {String} 마스킹처리된 숫자
 * @hidden N
 * @exception 
 * @example 
 * $c.num.formatUnit(32132);        //return "32,132";
 * $c.num.formatUnit(32132, 1000);  //return "32";
 */
scwin.formatUnit = function ($p, value, unit = '1') {
  if ($c.num.isNumber($p, value)) {
    value = $c.num.floor($p, value / unit, 0);
 ...
 * @method
 * @name formatDisplay
 * @description 보여지는 format 형식을 지정
 * @param {String} value 숫자
 * @param {String} format 형식
 * @returns {String} format처리된 문자열. 소수점은 반올림처리
 * @hidden N
 * @exception 
 * @example 
 * $c.num.formatDisplay(32132, "#,###"");       //return "32,132";
 * $c.num.formatDisplay(100, "#,###.##");       //return "100";
 * $c.num.formatDisplay(100, "#,###.#0");       //return "100.00";
 * $c.num.formatDisplay(1000.456, "#,###.#0");  //return "1,000.46";
 */
scwin.formatDisplay = function ($p, value, format) {
  if ($c.num.isNumber($p, value)) {
    return WebSquare.text.formatCurrency(value, format);
  }
  return value;
};
