/*amd /cm/gcc/win.xml 51902 b298e697451b9f8d674a68b1eba6eb4689915941648cc3cbf7acf06c20efc9da */
define({declaration:{A:{version:'1.0',encoding:'UTF-8'}},E:[{T:1,N:'html',A:{xmlns:'http://www.w3.org/1999/xhtml','xmlns:ev':'http://www.w3.org/2001/xml-events','xmlns:w2':'http://www.inswave.com/websquare','xmlns:xf':'http://www.w3.org/2002/xforms'},E:[{T:1,N:'head',A:{},E:[{T:1,N:'w2:type',E:[{T:3,text:'COMMON'}]},{T:1,N:'w2:buildDate'},{T:1,N:'w2:MSA'},{T:1,N:'xf:model',E:[{T:1,N:'w2:dataCollection',A:{baseNode:'map'}},{T:1,N:'w2:workflowCollection'}]},{T:1,N:'w2:layoutInfo'},{T:1,N:'w2:publicInfo',A:{method:'scwin.__getI18NUrl,scwin.getScope,scwin.__getScope,scwin.getActiveWindowInfo,scwin.showToastMessage,scwin.openMenu,scwin.openPage,scwin.openPageCode,scwin.openPopup,scwin.closePopup,scwin.closeAllPopup,scwin.isPopup,scwin.messagBox,scwin.setProgramAuthority,scwin.processCommonData,scwin.setHistoryBackEvent,scwin.pushState,scwin.changePageState,scwin.addEventOnBeforeUnload,scwin.removeEventOnBeforeUnload,scwin.__setOnBeforeUnload,scwin.errorHandler,scwin.reload,scwin.getProgramId,scwin.goHome,scwin.logout,scwin.isAdmin,scwin.getFullPath,scwin.setEnterKeyEvent,scwin.alert,scwin.confirm,scwin.getLanguage,scwin.getPopupId,scwin.moveUrl,scwin.setFrameSrc,scwin.getFrame,scwin.getParent,scwin.setLangCode,scwin.getLangCode,scwin.getCbFunctionManager'}},{T:1,N:'script',A:{lazy:'false',type:'text/javascript'},E:[{T:4,cdata:function(scopeObj){with(scopeObj){/**
 * @component
...
scwin.getScope = function ($p, comObj) {
  if (typeof comObj == 'undefined') {
    const scopeApi = $p;
    return scwin.__getScope(scopeApi);
  }
  try {
...
scwin.getActiveWindowInfo = function ($p, scopeApi) {
  ...
  if (popupWindow !== null) {
    ...
    activeInfo["type"] = "P";
    activeInfo["programCd"] = scwin.getProgramId($p, popupWindow.$p);
    ...
    if (activeInfo["type"] == "T") {
      ...
      findProgramList = $c.data.menuComp.getMatchedJSON("MENU_C", selectedTabId, true);
      ...
      findProgramList = $c.data.menuComp.getMatchedJSON("MENU_C", selectedWindowId, true);
      if (findProgramList.length > 0) {
        activeInfo["programCd"] = findProgramList[0].PROGRAM_CD;
      }
    ...
    activeInfo["type"] = "P";
    activeInfo["programCd"] = scwin.getProgramId($p);
...
scwin.openMenu = function ($p, menuNm, url, menuCode, paramObj, option) {
  ...
  if ($c.util.isEmpty($p, url)) {
    scwin.alert($p, "메뉴에 프로그램이 등록되지 않았습니다.");
    return false;
  }
  if (url == "/") {
    ...
    // url = $c.sbm.getContextPath() + "/ui" + url + ".xml";
    ...
    const options = {
      ...
      windowTitle: menuNm,
      windowId: menuCode,
      openAction: option.openAction || "selectWindow",
      //frameMode : "wframe",
      frameMode: "pageFrame",
 ...
 * @exception 
 * @example
 * $c.win.openPage("/tmp/tmp01.xml","010001", "차트화면");
 */
scwin.openPage = function ($p, url, menuCode, menuNm, paramObj) {
  if ($c.util.isEmpty($p, paramObj)) {
    paramObj = {};
  }

  // menuCode가 dlt_menu 에 존재하는 경우 dept는 dlt_menu 거로 / 없으면 호출한 화면의 dept + menuNm
  const menuInfoList = $p.main().pfm_side.getWindow().dlt_menu.getMatchedJSON("MENU_C", menuCode);
  if (menuInfoList.length < 1) {
    paramObj.parentMenuCd = $c.data.getParameter($p, "menuInfo").menuCode;
    option.menuType = "NONE"; //메뉴경로가 없는 화면
    option.pgmID = menuInfoList; //프로그램ID
  }
  let option = {
    //화면 오픈 옵션
    isHistory: true,
    //Browser History에 기록할 지 여부 (true, false)
    closable: true,
    //닫기버튼 보여주기 여부
    pgmID: menuInfoList[0].PGID //프로그램ID
  };
  scwin.openMenu($p, menuNm, url, menuCode, paramObj, option);
};

/**
 * @method 
 * @name openPageCode
 * @description 메뉴코드에 해당하는 메뉴를 재오픈한다.
 * @param {String} menuCode 메뉴코드 - DB에 저장되어있는 메뉴 코드
 * @param {Object} paramObj 화면에 전달할 JSON 데이터 객체
 * @hidden N
 * @exception 
 * @example
 * $c.win.openPageCode("010001", {paramObj : {key:"test1"}});
 */
scwin.openPageCode = function ($p, menuCode, paramObj) {
  if ($c.util.isEmpty($p, paramObj)) {
    paramObj = {};
  }
  ...
  if (menuInfoList.length == 0) {
    $c.win.alert($p, "메뉴코드 (" + menuCode + ")에 해당하는 메뉴정보가 없습니다");
    return;
  }
  const menuNm = menuInfoList[0].MNNM;
  const url = menuInfoList[0].PGM_PATHNM;
  const option = {
    //화면 오픈 옵션
    isHistory: true,
    //Browser History에 기록할 지 여부 (true, false)
...
scwin._openPopup = function ($p, url, opt, data) {
  ...
  //bbok
  if (!$c.util.isEmpty($p, opt.popSize)) {
    width = popupSize[opt.popSize].width;
    height = popupSize[opt.popSize].height;
  }
  try {
  ...
  const options = {
    ...
    closeAction: function () {
      ...
      const messageType = $c.data.getParameter($p, "messageType", popupWindow.$p) || "";
...
scwin.closePopup = function ($p, callbackParam, callbackFnStr) {
  ...
  scwin._closePopup($p, scwin.getPopupId($p), $c.str.serialize($p, callbackParam), callbackFnStr);
};
  ...
  $c.util.setTimeout($p, function () {
    if ($p.isWFramePopup()) {
      ...
      if (func) {
        ...
        //callbackFnStr이 없으나 callbackFn 이 있으면 callback 처리해줌.
        if ($c.util.isEmpty($p, callbackFnStr) && !$c.util.isEmpty($p, $c.data.getParameter($p, "callbackFn"))) callbackFnStr = $c.data.getParameter($p, "callbackFn");
...
scwin.messagBox = function ($p, messageType, messageStr, closeCallbackFncName, opts) {
  ...
  scwin.openPopup($p, "/cm/main/messageBox.xml", options, data);
};
...
scwin.setProgramAuthority = function ($p) {
  ...
  if (typeof menuInfo !== "undefined" && typeof menuInfo.menuCode !== "undefined" && menuInfo.menuCode.trim() !== "") {
    ...
    const menuInfoList = $p.main().pfm_side.getWindow().dlt_menu.getMatchedJSON("MENU_C", menuCd);
    if (menuInfoList.length > 0) {
...
scwin.setHistoryBackEvent = function ($p) {
  if (window.addEventListener) {
    window.addEventListener("popstate", scwin.changePageState);
  } else {
    window.attachEvent("popstate", scwin.changePageState);
  }
};
...
scwin.pushState = function ($p, data) {
  if (!scwin.isHistory) return; //bbok 강제로 return
...
scwin.__changePageState = function () {
  if (!$c.util.isEmpty($p, history.state) && !$c.util.isEmpty($p, history.state.data) && !$c.util.isEmpty($p, history.state.data.menuInfo)) {
    ...
    scwin.openMenu($p, data.menuInfo.menuNm, data.menuInfo.src, data.menuInfo.menuCode, data.param, options);
  }
};
...
scwin.addEventOnBeforeUnload = function ($p) {
  if (window.addEventListener) {
    window.addEventListener("beforeunload", scwin.__setOnBeforeUnload);
  } else {
    window.attachEvent("onbeforeunload", scwin.__setOnBeforeUnload);
  }
};
...
scwin.removeEventOnBeforeUnload = function ($p) {
  if (window.removeEventListener) {
    window.removeEventListener("beforeunload", scwin.__setOnBeforeUnload);
  } else {
    window.detachEvent("onbeforeunload", scwin.__setOnBeforeUnload);
  }
};
...
scwin.errorHandler = function ($p, e) {
  if (e.status == 404) {
    scwin.alert($p, e.responseURL + " URL이 존재하지 않습니다.");
  }
};
...
scwin.reload = function ($p) {
  scwin.removeEventOnBeforeUnload($p);
...
scwin.alert = function ($p, messageStr, closeCallbackFncName, opts) {
  return new Promise(function (resolve, reject) {
    ...
    scwin.messagBox($p, "alert", messageStr, closeCallbackFncName, opts);
  });
};
...
scwin.confirm = function ($p, messageStr, closeCallbackFncName, opts) {
  return new Promise(function (resolve, reject) {
    ...
    scwin.messagBox($p, "confirm", messageStr, closeCallbackFncName, opts);
  });
};
...
scwin.getLanguage = function ($p) {
  const language = scwin.getLangCode($p) || navigator.language || navigator.userLanguage || navigator.systemLanguage;
 ...
 * @param {Object} frameObj WFrame 객체
...
$c.win.setFrameSrc(wfm_content, "/tmp/tmp01.xml", param);
 */
scwin.setFrameSrc = function ($p, frameObj, moveUrl, paramObj) {
  paramObj = {
    "dataObject": {
      "type": "json",
      "name": "paramData",
  ...
  const contextPath = $c.sbm.getContextPath($p);
  return frameObj.setSrc(contextPath + moveUrl, paramObj);
};
